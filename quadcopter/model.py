...
class QuadcopterObserverModel(ObserverModel):
    def __init__(self):
        ...
        state_eq_constraints = [(lambda x: np.array([x[0]**2 + x[1]**2 + x[2]**2 + x[3]**2 - 1]),
                                 1,
                                 lambda x: np.array([[2*x[0], 2*x[1], 2*x[2], 2*x[3], 0, 0, 0, 0, 0, 0]]))]
    ...
    def derive_lambdas(self):
        a, b, c, d, bwx, bwy, bwz, bfx, bfy, bfz, wx, wy, wz, h = sp.symbols('a b c d bwx bwy bwz bfx bfy bfz omega_x omega_y omega_z h')
        wrealx = wx-bwx
        wrealy = wy-bwy
        wrealz = wz-bwz
